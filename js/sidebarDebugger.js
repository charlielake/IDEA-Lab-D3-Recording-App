...
function initializeSession() {
  ...
  session.on('streamCreated', function(event) {
    ...
    SubHeading.innerHTML += '<p>' + 'streamCreated Event' + '</p>';
  ...
  session.on('archiveStarted', function archiveStarted(event) {
      ...
      SubHeading.innerHTML += '<p>' + 'archiveStarted Event' + '</p>';
      recordingStatusSidebar = true;
      updateButtonDisplay();
      updateD3();
    });
  ...
  session.on('archiveStopped', function archiveStopped(event) {
      ...
      SubHeading.innerHTML += '<p>' + 'archiveStopped Event' + '</p>';
      recordingStatusSidebar = false;
      updateButtonDisplay();
      updateD3();
  ...
  session.on('sessionDisconnected', function sessionDisconnected(event) {
      ...
      SubHeading.innerHTML += '<p>' + 'sessionDisconnected Event' + '</p>';
    });
  
    // Connect to the session
  session.connect(token, function connectCallback(error) {
      // If the connection is successful, initialize a publisher and publish to the session
      SubHeading.innerHTML += '<p>' + 'sessionConnectCallback Event' + '</p>';
      if (!error) {
        ...
        var publisher = OT.initPublisher('publisher', publisherOptions, function initCallback(err) {
          SubHeading.innerHTML += '<p>' + 'Init Publisher' + '</p>';
          if (err) {
            console.log('There was an error initializing the publisher: ', err.name, err.message);
            SubHeading.innerHTML += '<p>' + 'There was an error initializing the publisher:' + err.name + err.message + '</p>';
          ...
          session.publish(publisher, function publishCallback(pubErr) {
            if (pubErr) {
              ...
              SubHeading.innerHTML += '<p>' + 'There was an error publishing:' + '</p>';
            }
          });
        });
      } else {
        console.log('There was an error connecting to the session: ', error.name, error.message);
        SubHeading.innerHTML += '<p>' + 'There was an error connecting to the session' + '</p>';
      }
    });
    //stopArchive();
}
...
function startArchive() { // eslint-disable-line no-unused-vars
  var SubHeading = q("Recording Status Sidebar");
  var currentButtonDisplay = q("Recording Button Sidebar");
  SubHeading.innerHTML += '<p>' + 'Start of StartArchive()' + '</p>';
  ...
  function stopArchive() { // eslint-disable-line no-unused-vars
    var currentButtonDisplay = q("Recording Button Sidebar");
    var SubHeading = q("Recording Status Sidebar");
    SubHeading.innerHTML += '<p>' + 'Start of stopArchive()' + '</p>';
    
    $.post(SAMPLE_SERVER_BASE_URL + '/archive/' + archiveID + '/stop');
    recordingStatusSidebar = false;
    updateButtonDisplay();
   // update D3 if archive was started from the sidebar-side of the call
   //if (sentFromD3 == false) {
    updateD3();
   // }
   // Enable view button, as archive will now be available
    $('#view').prop('disabled', false);
    SubHeading.innerHTML += '<p>' + 'End of stopArchive()' + '</p>';
    }
    ...
    function viewArchive() { // eslint-disable-line no-unused-vars
      ...
      SubHeading.innerHTML += '<p>' + 'End of viewArchive()' + '</p>';
    }